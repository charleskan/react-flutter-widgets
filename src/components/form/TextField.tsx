...
function mapKeyboard(type?: TextInputType): { htmlType: string; inputMode?: React.HTMLAttributes<HTMLInputElement>["inputMode"]; } {
  switch (type) {
    case "emailAddress":
...
export const TextField = forwardRef<TextFieldHandle, TextFieldProps>(function TextField(
  ...
  useImperativeHandle(ref ?? forwardedRef, (): TextFieldHandle => ({
  ...
  const ta = textAlign === "start" ? undefined : textAlign === "end" ? undefined : (textAlign as any);
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    if (e.key === "Enter") {
      ...
      if (textInputAction === "next") {
        ...
        if (form) {
          const elements = Array.from(form.querySelectorAll<HTMLElement>("input, textarea, [tabindex]"));
  ...
  return (
    <label className={"rtf-container " + (className ?? "")} style={{ display: "block", ...containerStyle }}>
      {labelText && (
        <span className="rtf-label">{labelText}</span>
      )}
      <div
